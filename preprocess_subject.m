function [ALLEEG, EEG_preprocessed, CURRENTSET] = preprocess_subject(subjects, bemobil_config, force_recompute)
    for subject = subjects
	    ...
	    recording_folders = dir(fullfile(input_filepath,'*.set'));
        recordings = cell(1,height(recording_folders));
    
        
        for k=1:height(recording_folders)
            recordings{k} = recording_folders(k).name;
        end
    
        recordings_idxs = [];
    
        if ~isempty(bemobil_config.recordings)
            for i=1:length(bemobil_config.recordings)
                recordings_idxs = [recordings_idxs;find(contains(recordings,['_' bemobil_config.recordings{i}]))];
            end
        else
        ...
        for j = 1:length(recordings_idxs)
            recording = recordings{recordings_idxs(j)};
	        try
		        ...
		        EEG_single_subject_final = pop_loadset('filename', [ bemobil_config.filename_prefix num2str(subject)...
			        '_' recording.name '_' erase(bemobil_config.preprocessed_and_ICA_filename,'.set') '_filtered.set'], 'filepath', output_filepath);
            catch
            ...
            EEG = pop_loadset('filename',recording,'filepath',input_filepath);
            ...
            % save plot
            print(gcf,fullfile(input_filepath,[erase(EEG.filename,'.set') '_raw-full.png']),'-dpng')
            close
